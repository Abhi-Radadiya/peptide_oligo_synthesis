...
export default function ToggleSwitch(props) {
    const { checked, label, handleChange, disabled, offBgColor = "bg-neutral-200", checkedBgColor = "checked:bg-slate-800", className, id } = props

    return (
        <>
            <div className={`flex flex-row gap-2 items-center ${className}`}>
                {label && (
                    <label htmlFor={id} className="text-gray-700 leading-[17px] font-normal">
                        {label}
                    </label>
                )}
