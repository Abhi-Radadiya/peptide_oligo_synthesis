...
import { removePump, updatePump } from "../../../../../redux/reducers/settings/hardwareSetup"

export default function SinglePumpBlock(props) {
    const { pump, pumpName } = props
    ...
    const handleRemovePump = () => {
        dispatch(removePump({ pumpId: pump.pumpId }))

        setShowDeleteConfirmationModel(null)
    }

    const { control, handleSubmit, setValue } = useForm({ defaultValues: { pumpName: "" } })

    const validateUniquePumpName = (value) => {
        // Check if the value already exists in any bottle
        const isUnique = !containerBottles.pump.some((el) => el.pumpName === value)

        // Return an error message if the name is not unique
        return isUnique || "Pump name must be unique"
    }

    const inputName = `pumpName.${pump.index}`

    const handleModify = () => {
        setValue(inputName, pumpName)
        setIsModify(true)
    }
    ...
    const handleSaveName = (data) => {
        dispatch(updatePump({ pumpName: data.pumpName[pump.index], pumpId: pump.pumpId }))
    ...
    return (
        <>
                ...
                <div className="flex items-center flex-1 min-w-0">
                    ...
                    {isModify ? (
                        <InputField
                            ...
                            rules={{ required: "Bottle name is required", validate: validateUniquePumpName }}
                            inputRef={inputRef}
                            wrapperClassName="w-full"
                        />
                    ) : (
                        <span className="font-normal text-sm">
                            Name : <strong className="">{pumpName}</strong>
                        </span>
                    )}
                    ...
                    <span className="font-normal text-sm flex-shrink-0">
                        Valve : <strong>{pump?.index}</strong>
                    </span>
                </div>
            ...
            <ConfirmationPopup
                ...
                handleConfirm={handleRemovePump}
            />
        </>
    )
}
