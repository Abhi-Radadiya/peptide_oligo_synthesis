...
const ReagentConfiguration = () => {
    ...
    const updateDetails = async (data) => {
        try {
            const response = await updateBottleMappingDetails(data, "reagent");
            if (response.success) {
                loadAmediteList();
    ...
    const addNewDetails = async (data) => {
        try {
            const response = await saveBottleMappingDetails(data, "reagent");
    ...
    const handleDelete = async (id) => {
        try {
            await deleteBottleMappingDetails(id, "reagent");
            loadAmediteList();
        } catch (error) {
            console.log(`error : `, error);
        }
    ...
    return (
        <div className="overflow-x-auto p-4 border border-neutral-300 rounded-xl bg-neutral-50">
            <div className="flex flex-row justify-between items-center mb-4">
                <h1 className="text-xl font-medium">Reagent</h1>
