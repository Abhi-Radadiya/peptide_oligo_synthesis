...
export default function Sulfurization() {
        ...
        <>
            <div className="flex flex-row w-full items-start justify-between border border-neutral-300 rounded-2xl shadow-md py-4 px-6 mb-6">
                <div className="max-w-[615px] w-full">
                    ...
                    <div className="flex flex-row items-center justify-between mb-4">
                        <div className="flex flex-row items-center gap-6 w-full  justify-between">
                            <SelectionController width={200} menuItem={testSolvents} control={control} name="sulfurization_solvent" placeholder="Select Solvent" />
                            ...
                            <p className="w-full max-w-[220px] border border-neutral-300 rounded-lg px-4 py-2">
                                ...
                                <span>{watch("sulfurization_solvent")?.flowRate ?? "___  "} ml/min</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-row items-center justify-between my-4">
                        <div className="flex flex-row items-center gap-6 w-full justify-between">
                            <SelectionController width={200} menuItem={testSolvents} control={control} name="sulfurization_wash_solvent" placeholder="Select Solvent" />

                            <p className="w-full max-w-[220px] border border-neutral-300 rounded-lg px-4 py-2 shadowd-lg">
                                <span className="font-bold text-base mr-2">Flow Rate : </span>
                                <span>{watch("sulfurization_wash_solvent")?.flowRate ?? "___  "} ml/min</span>
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-row items-center gap-6 justify-between border-b border-neutral-300 pb-4 mb-4">
                        <span className="font-bold text-neutral-600">X Factor</span>
                        <InputField name="sulfurization_wash_x_factor" width="w-[220px]" control={control} type="number" placeholder="Enter X Factor" />
                    </div>

                    <div className="flex flex-row justify-between items-center w-full">
                        <div className="flex flex-row items-center gap-6 justify-between">
                            <span className="font-bold text-neutral-600">Conductivity</span>
                            <RadioButton className="max-w-[250px]" disabled buttons={buttons} control={control} name="sulfurization_conductivity" />
                        </div>
                ...
                <div className="flex flex-row w-full max-w-[260px] items-center gap-3">
                    ...
                    <SelectionController isClearable={false} width={120} menuItem={wasteMenuItems} control={control} name="sulfurization_waste" />
                </div>
            </div>
        </>
    );
